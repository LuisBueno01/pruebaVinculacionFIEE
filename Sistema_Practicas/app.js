...
folioForm.addEventListener("submit", async (e) => {
  ...
  try {
    await addDoc(collection(db, "foliosestadias"), {
      folio,
      nombre,
      empresa,
      carrera,
...
btnEdit.onclick = async () => {
  ...
  const dependencia = prompt("Nueva empresa:", registroActual.empresa);
  if (!empresa) return;
...
function actualizarEstadisticas() {
  ...
  document.getElementById("totalempresa").textContent =
